...
export default function ImportData() {
  ...
  const handleImport = () => {
    ...
    try {
      // Automatisches Aufräumen: Entfernt ```json ... ``` und Leerzeichen
      ...
      setParseError('ungültiges json-format. bitte prüfe die eingabe (z.b. fehlende anführungszeichen oder kommas).');
    }
  };
...
- Kleinbuchstaben verwenden (marthi-style).
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-12 pb-20">
      <div className="text-center space-y-4">
        <h1 className="text-7xl font-display text-[#8B1E1E] lowercase">daten-import</h1>
        <p className="text-xl text-[#4A3A2F]/50 font-medium font-sans max-w-2xl mx-auto lowercase italic">
        ...
        <div className="lg:col-span-7 space-y-8">
          <div className="bg-white/60 backdrop-blur-md p-8 rounded-[48px] shadow-xl border border-white space-y-6">
            <div className="flex items-center justify-between">
               <h2 className="text-3xl font-display text-[#8B1E1E] flex items-center gap-3 lowercase">
                  <FileJson className="w-6 h-6" /> json eingabe
               </h2>
               <button 
                 onClick={() => { setJsonInput(''); setParseError(null); }}
                 className="text-[10px] font-black uppercase tracking-widest text-[#4A3A2F]/20 hover:text-[#8B1E1E] transition-colors lowercase"
               >
                 leeren
               </button>
            </div>
            ...
            <textarea
              ...
              className="w-full h-[400px] p-6 bg-[#E2E8D4]/30 border border-black/[0.03] rounded-[32px] focus:outline-none focus:bg-white focus:ring-4 focus:ring-[#8B1E1E]/5 transition-all font-mono text-sm leading-relaxed lowercase"
            />

            <button
              onClick={handleImport}
              disabled={!jsonInput.trim() || importMutation.isPending}
              className="w-full py-6 bg-[#8B1E1E] text-white rounded-[24px] font-display text-3xl shadow-xl hover:bg-[#763428] transition-all flex items-center justify-center gap-4 disabled:opacity-20 disabled:grayscale lowercase"
            >
            ...
            <AnimatePresence>
              {importMutation.isSuccess && (
                ...
                >
                  ...
                  <div className="text-sm font-bold lowercase">import erfolgreich! deine app ist nun klüger geworden.</div>
                </motion.div>
              )}
              ...
              {(parseError || importMutation.isError) && (
                ...
                >
                  ...
                  <div className="space-y-1">
                    <div className="text-sm font-bold lowercase">fehler beim import</div>
                    <p className="text-xs opacity-80 lowercase">{parseError || importMutation.error?.message || 'unbekannter fehler'}</p>
                  </div>
                </motion.div>
              )}
            </AnimatePresence>
        ...
        <div className="lg:col-span-5 space-y-8">
           <div className="bg-white/40 backdrop-blur-sm p-8 rounded-[48px] border border-white space-y-8 relative overflow-hidden shadow-sm">
              ...
              <div className="space-y-2 relative z-10">
                 <div className="flex items-center gap-2 text-[#AF4D3C] font-black uppercase tracking-widest text-[10px] lowercase">
                    <Sparkles className="w-3 h-3" /> ki-helfer
                 </div>
                 <h3 className="text-4xl font-display text-[#8B1E1E] lowercase">ki-assistent nutzen</h3>
              </div>

              <p className="text-[#4A3A2F]/60 text-sm font-medium font-sans leading-relaxed relative z-10 lowercase">
              ...
              >
                 <span className="font-display text-2xl text-[#8B1E1E] lowercase">prompt kopieren</span>
              ...
              <div className="space-y-4 pt-4 border-t border-black/[0.03]">
                 <h4 className="text-[10px] font-black uppercase tracking-widest text-[#4A3A2F]/30 lowercase">format-vorschau</h4>
                 <div className="grid grid-cols-3 gap-3">
                    <div className="bg-[#E2E8D4]/40 p-4 rounded-2xl text-center">
                       <BookOpen className="w-5 h-5 mx-auto mb-2 text-[#8B1E1E]/40" />
                       <div className="text-[10px] font-black uppercase text-[#4A3A2F]/40 lowercase">bibliothek</div>
                    </div>
                    <div className="bg-[#E2E8D4]/40 p-4 rounded-2xl text-center">
                       <List className="w-5 h-5 mx-auto mb-2 text-[#A3B18A]/40" />
                       <div className="text-[10px] font-black uppercase text-[#4A3A2F]/40 lowercase">fragen</div>
                    </div>
                    <div className="bg-[#E2E8D4]/40 p-4 rounded-2xl text-center">
                       <Brain className="w-5 h-5 mx-auto mb-2 text-[#E9C46A]/40" />
                       <div className="text-[10px] font-black uppercase text-[#4A3A2F]/40 lowercase">karten</div>
                    </div>
                 </div>
              </div>
           </div>
           ...
           <div className="bg-white/40 p-8 rounded-[40px] border border-black/[0.02] space-y-4 shadow-sm">
              <h4 className="text-xl font-display text-[#4A3A2F] flex items-center gap-2 lowercase">
                 <Info className="w-4 h-4" /> tipps für den import
              </h4>
              <ul className="space-y-3 text-xs text-[#4A3A2F]/60 font-medium font-sans list-disc pl-4 lowercase italic">
