...
export default function MistakeGarden() {
  ...
  const handleNext = () => {
    if (selectedOption === activeMistake?.correctAnswerIndex) {
      // If correct, remove from garden immediately as requested
  ...
  return (
    <div className="max-w-5xl mx-auto space-y-12 pb-20">
      <div className="text-center space-y-4">
        <div className="inline-flex items-center gap-2 px-4 py-1.5 bg-[#DCE2C8] rounded-full text-[#344E41] text-xs font-black uppercase tracking-[0.2em] lowercase">
           <Sprout className="w-3 h-3" /> wachstums-zone
        </div>
        <h1 className="text-7xl font-display text-[#8B1E1E] lowercase">fehler-garten</h1>
        <p className="text-xl text-[#4A3A2F]/50 font-medium font-sans max-w-2xl mx-auto italic lowercase">
      ...
      {mistakes.length === 0 ? (
        <div className="bg-[#F9F4E8] p-20 rounded-2xl text-center border border-[#4A3A2F]/8 flex flex-col items-center gap-6" style={{ boxShadow: '2px 2px 0 rgba(74,58,47,0.07)' }}>
           ...
           <h2 className="text-4xl font-display text-[#8B1E1E] lowercase">dein garten blüht prächtig!</h2>
           <p className="text-[#4A3A2F]/50 font-sans text-lg lowercase">keine fehler mehr übrig. zeit für eine neue klausur?</p>
        </div>
      ) : (
            ...
            {mistakes.map((m) => {
              ...
              return (
                ...
                >
                  ...
                  <h3 className="text-xl font-bold text-[#4A3A2F] leading-tight line-clamp-3 mb-4 font-sans lowercase">{m.text.toLowerCase()}</h3>
                  
                  <div className="mt-auto pt-4 border-t border-black/[0.03]">
                     <span className="text-[10px] font-black uppercase tracking-widest text-[#4A3A2F]/20 lowercase">{subject?.name.toLowerCase() || 'fach'}</span>
                  </div>
                </motion.button>
              );
            })}
            ...
            >
              ...
              <div className="p-10 md:p-14 overflow-y-auto custom-scrollbar space-y-8">
                <div className="flex items-center justify-between mb-4">
                   <div className="text-[10px] font-black uppercase tracking-[0.3em] text-[#8B1E1E]/40 lowercase">frage korrigieren</div>
                   {!isAnswered && (
                ...
                <h2 className="text-3xl md:text-4xl font-display text-[#8B1E1E] leading-tight text-center mb-8 lowercase">{activeMistake.text.toLowerCase()}</h2>
                  ...
                  {activeMistake.options.map((opt, idx) => {
                    ...
                    return (
                      <button
                        ...
                        disabled={isAnswered}
                        className={cn(
                          "w-full text-left p-6 rounded-[24px] font-sans text-lg font-bold transition-all flex items-center justify-between border-2 lowercase",
                          classes
                        )}
                      >
                        <span className="pr-4">{opt.toLowerCase()}</span>
                  ...
                  {isAnswered && (
                    ...
                    >
                      <div className="bg-[#E2E8D4]/50 p-6 rounded-3xl border border-black/[0.02]">
                         <h4 className="text-[10px] font-black uppercase text-[#8B1E1E] mb-3 lowercase">erklärung</h4>
                         <p className="text-lg text-[#4A3A2F]/80 leading-relaxed font-serif italic lowercase">"{activeMistake.explanation.toLowerCase()}"</p>
                      </div>
                      <button 
                        onClick={handleNext}
                        className="w-full py-6 bg-[#8B1E1E] text-white rounded-3xl font-display text-3xl shadow-xl hover:bg-[#763428] transition-all flex items-center justify-center gap-4 lowercase"
                      >
